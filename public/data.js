export const portfolioData = [
  {
    title: "Qizify",
    subTitle:
      "A quiz game application to play with your friends in different languages.",
    background:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604652911/ilnweb/circle1_evxgd9.png",
    desctop:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1602862830/ilnweb/Screenshot_221_rk9fxf.png",
    mobile:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1602862830/ilnweb/Screenshot_2020-10-16_173816_weptbq.png",
    description:
      "Qizify lets you play the games in three different languages. At the moment we support English German and Russian. Ones you purchaise a game you can play with your non fuent frinds and switch the language while you play. The project was inspired by game rooms where Qiuz fans are gathering togheder to compete and are in constant need of new games.",
    buttonName: "View project",
    link: "https://www.qizify.com/",
  },
  {
    title: "Devster",
    subTitle:
      "Social media application inspired by my favorite sited Medium and DEV.",
    background:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604651204/ilnweb/triangle1_x9yzum.png",
    desctop:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604675232/ilnweb/Screenshot_231_df5gsn.png",

    mobile:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604675650/ilnweb/Screenshot_233_vgkwya.png",

    description:
      "Dev SOcial Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap",
    buttonName: "View code",
    link: "https://github.com/ilnweb/dev-social",
  },
  {
    title: "Togheder",
    subTitle:
      "An application where you and your friends can keep track of mutual spendings.",
    background:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604654596/ilnweb/squaire1_nszow7.png",
    desctop:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604689549/ilnweb/Screenshot_236_fldtxr.png",
    mobile:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604689549/ilnweb/Screenshot_235_iwf3pi.png",
    description:
      "Togheder Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap",

    mobileOnly: true,
    buttonName: "View project",
    link: "https://together-live.netlify.app/",
  },
  {
    title: "Crown clothing",
    subTitle: "Ecommerse template perfect for small bussiness.",
    background:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604651204/ilnweb/triangle1_x9yzum.png",
    desctop:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1602862830/ilnweb/Screenshot_221_rk9fxf.png",
    mobile:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1602862830/ilnweb/Screenshot_2020-10-16_173816_weptbq.png",
    description:
      "Crown Clothing is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap",
    buttonName: "View code",
    link: "https://github.com/ilnweb/crwn-clothing",
  },
  {
    title: "Iliyan Tsachev photography",
    subTitle: "My personal photography page.",
    background:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604652911/ilnweb/circle1_evxgd9.png",
    desctop:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1602862830/ilnweb/Screenshot_221_rk9fxf.png",
    mobile:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1602862830/ilnweb/Screenshot_2020-10-16_173816_weptbq.png",
    description:
      "Crown Clothing is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap",
    buttonName: "View project",
    link: "https://iliyan-tsachev-live.netlify.app/",
  },
  {
    title: "The Design House",
    subTitle: "Dubai based inderior designer's showcase website.",
    background:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604652911/ilnweb/circle1_evxgd9.png",
    desctop:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604763196/ilnweb/Screenshot_240_jh5bj8.png",
    mobile:
      "https://res.cloudinary.com/ilnphotography/image/upload/v1604762934/ilnweb/Screenshot_238_uc6gic.png",
    description:
      "I created this Wordpress template for a Interior designer based in Dubai. Its fully customisable and the owner has control over the content and how it will be displayed. One of my favorite thing about this project is the simple and elegant design. Most of the photography was also done by me which I'm very proud of.",
    buttonName: "View project",
    link: "http://thedhdubai.com/",
  },
];
